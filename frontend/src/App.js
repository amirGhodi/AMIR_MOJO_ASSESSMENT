...
function App() {
  ...
  const responseFacebook = async (response) => {
    setUser(response);
    try {
      const res = await axios.post(`${backendURL}/api/facebook/pages`, { accessToken: response.accessToken });
  ...
  const getPageAccessToken = async (pageId) => {
    try {
      const response = await axios.get(`https://graph.facebook.com/v12.0/${pageId}`, {
        params: {
          fields: 'access_token',
        ...
        switch (insight.name) {
          ...
          case 'page_impressions':
            ...
            case 'page_actions_post_reactions_total':
              insights.totalReactions = Object.keys(insight.values[0]?.value || {}).reduce((sum, key) => {
